...
namespace RVP
{
    ...
    public class VehicleHud : MonoBehaviour
    {
        ...
        void Update() {
            if (vp) {
                speedText.text = (vp.velMag * 3.6f).ToString("0"); // + " KMH"; // #% (MPH: * 2.23694f)

                if (trans) {
                    if (gearbox) {
                        gearText.text = (gearbox.currentGear == 0 ? "R" : (gearbox.currentGear == 1 ? "N" : (gearbox.currentGear - 1).ToString())) + " | km/h"; // #%
                    }
                    else if (varTrans) {
