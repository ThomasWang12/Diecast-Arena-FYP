...
namespace RVP
{
    ...
    public class CameraControl : MonoBehaviour
    {
        ...
        [SerializeField] float displaceDistance = 1.0f;
        [Tooltip("Toggle smooth camera rotation when steering vehicle")]
        ...
        public void Initialize()
        {
            ...
            if (target)
            {
                ...
                aimPoint = target.Find("camlooktarget"); // #%
            }
        ...
        void FixedUpdate()
        {
            if (target && targetBody && target.gameObject.activeSelf)
            {
                ...
                else forwardLook = targetForward;

                lookObj.rotation = Quaternion.LookRotation(forwardLook, upLook);
                lookObj.position = target.position;

                Vector3 lookDirActual;
                if (smoothRotate) // #%
                    lookDirActual = (lookDir - Mathf.Abs(smoothYRot) * 0.2f * new Vector3(Mathf.Sin(smoothYRot), 0, Mathf.Cos(smoothYRot))).normalized;
                else lookDirActual = lookDir.normalized;

                Vector3 forwardDir = lookObj.TransformDirection(lookDirActual);
                Vector3 localOffset = lookObj.TransformPoint(-lookDirActual * distance - lookDirActual * Mathf.Min(targetBody.velocity.magnitude * 0.05f, 2) * displaceDistance + Vector3.up * height); // #%
                ...
                if (aimPoint) tr.LookAt(aimPoint); // #% Look at camlooktarget
                else tr.rotation = Quaternion.LookRotation(forwardDir, lookObj.up);
            }
        }

        public void ToggleSmooth(bool input) //#%
        {
            smoothRotate = input;
        }

        // Set the rotation input of the camera
